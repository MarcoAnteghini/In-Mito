...
matrix,inner,trans,outer = [],[],[],[]
for uniprotid in final_d:
    pred = LR_model.predict(final_d[uniprotid].reshape(1, -1))[0]
    if pred==0:
        matrix.append(uniprotid)
    if pred==1:
        inner.append(uniprotid)
    if pred==2:
        trans.append(uniprotid)
    if pred==3:
        outer.append(uniprotid)
print('\n'*2)
print('Matrix proteins: ',matrix)
print('Inner Membrane proteins: ',inner)
print('Inter-membrane-space proteins: ',trans)
print('Outer Membrane proteins:', outer)
